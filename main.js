  ...
  contextMenu.items.forEach((btn, i) => {
    btn.click = () => {
      switch (i) {
        ...
        case 2:
          if (
            (minecraftProc && sessionManager.state === "launched") ||
            sessionManager.state === "launch"
          ) {
...
if (!gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    appTray = new Tray(
      path.join(__dirname, "src", "assets", "icons", "icon_64.png")
    );
...
async function updateIP() {
  try {
    const r = await fetch(
      "https://raw.githubusercontent.com/pazzazzo/mccitizens-clientpackage/refs/heads/main/ip.txt"
    );
    if (r.ok) {
...
function fetchServerStatus() {
  ...
  return new Promise((resolve) => {
    ...
    socket.once("data", (data) => {
      if (!data || !data.length)
        return settle({ online: false, reason: "empty-response" });
...
function pathToDataURL(filePath) {
  ...
  const mime = guessMime(path.extname(filePath));
  const b64 = buf.toString("base64");
  return `data:${mime};base64,${b64}`;
}

ipcMain.handle("screens.get", async () => {
  try {
    let images = [];
    let filePath = path.join(rootPath(), "screenshots");
    if (!fs.existsSync(filePath)) {
      return [];
    }
    let paths = fs.readdirSync(filePath);
    paths.forEach((img) => {
      const date = fs.statSync(path.join(filePath, img)).birthtime;
      images.push({
        name: img,
        date,
        data: pathToDataURL(path.join(filePath, img)),
      });
    });
    return images;
  } catch (err) {
...
ipcMain.handle("screen.save", async (event, name) => {
  const { filePath, canceled } = await dialog.showSaveDialog({
    title: "Enregistrer le screen",
    defaultPath: name,
  ...
  fs.copyFileSync(path.join(rootPath(), "screenshots", name), filePath);
  return true;
});

ipcMain.handle("getGames", () => {
  let games = [{ name: "MCCitizens", value: "mccitizens", selected: true }];
  if (isDev) {
    games.push({
      name: "Dev mode",
      value: "dev",
      selected: liveServer ? true : false,
    });
  }
  return games;
});
ipcMain.on("setGame", (event, game) => {
  if (game === "dev" && isDev) {
    ...
    mainWindow.loadURL(
      "http://localhost:5500/index.html?payload=" +
        encodeURIComponent(JSON.stringify(sessionManager.playerData))
    );
  } else {
    if (liveServer) {
      liveServer.shutdown();
      ...
      mainWindow.loadFile(path.join(__dirname, "src", "index.html"), {
        query: {
          payload: encodeURIComponent(
            JSON.stringify(sessionManager.playerData)
          ),
        },
      });
    }
...
ipcMain.on("mods.get", (event) => {
  ...
  return new Promise((resolve, reject) => {
    const worker = new Worker(
      path.join(__dirname, "workers", "mods-worker.js")
    );
...
ipcMain.on("launch", async () => {
  ...
  console.log(
    `[MCCitizens] Client package ${
      isClientPackageInstalled() ? "already" : "not"
    } installed`
  );

  /** @type {import("minecraft-launcher-core").ILauncherOptions} */
  let opts = {
    clientPackage: isClientPackageInstalled()
      ? null
      : "https://github.com/pazzazzo/mccitizens-clientpackage/releases/latest/download/clientpackage.zip",
    removePackage: true,
...
launcher.on("data", (e) => {
  ...
  if (
    e.indexOf("Building Processors") >= 0 ||
    e.indexOf("[Render thread/INFO]") >= 0
  ) {
    sessionManager.state = "launched";
