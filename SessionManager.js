...
class SessionManager extends EventEmitter {
    ...
    get playerData() {
        if (this.profile) {
            return { "skin": this.profile.skins[0].url, "cape": this.profile.capes[0]?.url, "head": `https://mc-heads.net/head/${this.profile.name}/left`, "username": this.profile.name, "state": this.state, "hydix": this.hydix.profile }
        } else {
            return {}
        }
    }
